...
#This is the large-sample variant, replacing the studentized
#reference distribution with the standard Normal. At Gamma=1, this
#recovers the method of Baiocchi et al (2010)
#######
#Input
######
...
ERSensitivityLS = function(Y, D, null = 0, alpha = 0.05, alternative = "greater", Gamma=1, nperm = 50000, Changepoint = T, SensitivityInterval = T)
{
  ...
  if(Changepoint == T)
  {
    proceed = ERSensitivityLS(Ytrue, Dtrue, null, alphatrue, alternative, Gamma=1, nperm, Changepoint = F, SensitivityInterval = F)$pval <= alpha
    change = 1
    if(proceed)
    {
      change = uniroot(ERChangepointLS, interval = c(1, 30), Y = Ytrue, D = Dtrue, null = null, alpha = alphatrue, alternative = alternative, nperm = nperm, extendInt = "upX")$root
    } 	
  }	
  if(SensitivityInterval == T)
  {
    ...
    {		
      ...
      {
        SLB = uniroot(ERSILS, interval = c(UB-4*sd(Ytrue)/sqrt(I), UB), extendInt = "yes", Gamma = Gamma[i], Y=Ytrue, D = Dtrue, alternative = "greater", alpha = alpha, nperm = nperm)$root
      }
      if(alternative == "less")
      {
        SUB = uniroot(ERSILS, interval = c(LB, LB + 4*sd(Ytrue)/sqrt(I)), extendInt = "yes", Gamma = Gamma[i], Y=Ytrue, D = Dtrue, alternative = "less", alpha = alpha, nperm = nperm)$root
      }
      if(alternative == "two.sided")
      {
        SLB = uniroot(ERSILS, interval = c(UB-4*sd(Ytrue)/sqrt(I), UB), extendInt = "yes", Gamma = Gamma[i], Y=Ytrue, D = Dtrue, alternative = "greater", alpha = alpha, nperm = nperm)$root
...
ERChangepointLS = function(Gamma, Y, D, null, alternative, alpha, nperm)
{
  alphachange = alpha
  if(alternative=="two.sided")
  {
  ...
  ERSensitivityLS(Y, D, null, alpha, alternative, Gamma, nperm, Changepoint = F, SensitivityInterval = F)$pval - alphachange 
}

ERSILS = function(null,  Gamma, Y, D, alternative, alpha, nperm)
{
  ERSensitivityLS(Y, D, null, alpha, alternative, Gamma, nperm, Changepoint = F, SensitivityInterval = F)$pval - alpha  
