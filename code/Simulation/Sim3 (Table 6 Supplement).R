nsim = 10000
source("ERSensitivityHet.R")
source("ERSensitivityPoP.R")
library(nbpMatching)
alph = .1

simSettings = letters[1:16]
EstSize = matrix(0, length(simSettings), 3)
CILength = matrix(0, length(simSettings), 3)

#settings a-p are each of the rows in Table 1 of the Supplement
for(ii in 1:length(simSettings))
{
  ...
  for(i in 1:nsim)
  {
    ...
    zet2 = YC-lambda*DC
    plarge = 1.5/(2.5)
    probs = plarge*(zet1 >= zet2) + (1-plarge)*(zet1 < zet2)
    Z1 = (runif(B) <= probs)
    Ydobs = YT*Z1 + YC*(1-Z1)
    Ddobs = DT*Z1 + DC*(1-Z1)
    sensstandard = ERSensitivityHet(Ydobs, Ddobs, X = NULL, null = lambda, alpha = 0.1, alternative = "two.sided", Gamma=1.5, nperm = 1000, Changepoint = F, SensitivityInterval = T)
    rejstandard[i] =  sensstandard$pval <= alph
    SIstandard[i,] = sensstandard$SensitivityInterval[-1]
    senslin = ERSensitivityHet(Ydobs, Ddobs, X = X, null = lambda, alpha = 0.1, alternative = "two.sided", Gamma=1.5, nperm = 1000, Changepoint = F, SensitivityInterval = T)
    rejlin[i] = senslin$pval <= alph
    SIlin[i,] = senslin$SensitivityInterval[-1]
    ind = indexPoP(X, X)
    sensPoP = ERSensitivityPoP(Ydobs, Ddobs, index = ind, null = lambda, alpha = 0.1, alternative = "two.sided", Gamma=1.5, nperm = 1000, Changepoint = F, SensitivityInterval = T)
