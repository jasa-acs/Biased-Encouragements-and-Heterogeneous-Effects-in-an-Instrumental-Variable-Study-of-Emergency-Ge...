...
ERSensitivity = function(Y, D, null = 0, alpha = 0.05, alternative = "greater", Gamma=1, nperm = 50000, Changepoint = T, SensitivityInterval = T)
{
  ...
  if(Changepoint == T)
  {
    proceed = ERSensitivity(Ytrue, Dtrue, null, alphatrue, alternative, Gamma=1, nperm, Changepoint = F, SensitivityInterval = F)$pval <= alpha
    change = 1
    if(proceed)
    {
      change = uniroot(ERChangepoint, interval = c(1, 30), Y = Ytrue, D = Dtrue, null = null, alpha = alphatrue, alternative = alternative, nperm = nperm, extendInt = "upX")$root
    } 	
  }	
  if(SensitivityInterval == T)
  {
    ...
    {		
      ...
      {
        SLB = uniroot(ERSI, interval = c(UB-4*sd(Ytrue)/sqrt(I), UB), extendInt = "yes", Gamma = Gamma[i], Y=Ytrue, D = Dtrue, alternative = "greater", alpha = alpha, nperm = nperm)$root
      }
      if(alternative == "less")
      {
        SUB = uniroot(ERSI, interval = c(LB, LB + 4*sd(Ytrue)/sqrt(I)), extendInt = "yes", Gamma = Gamma[i], Y=Ytrue, D = Dtrue, alternative = "less", alpha = alpha, nperm = nperm)$root
      }
      if(alternative == "two.sided")
      {
        SLB = uniroot(ERSI, interval = c(UB-4*sd(Ytrue)/sqrt(I), UB), extendInt = "yes", Gamma = Gamma[i], Y=Ytrue, D = Dtrue, alternative = "greater", alpha = alpha, nperm = nperm)$root
...
ERChangepoint = function(Gamma, Y, D, null, alternative, alpha, nperm)
{
  alphachange = alpha
  if(alternative=="two.sided")
  {
  ...
  ERSensitivity(Y, D, null, alpha, alternative, Gamma, nperm, Changepoint = F, SensitivityInterval = F)$pval - alphachange 
}

ERSI = function(null,  Gamma, Y, D, alternative, alpha, nperm)
{
  ERSensitivity(Y, D, null, alpha, alternative, Gamma, nperm, Changepoint = F, SensitivityInterval = F)$pval - alpha  
  
}
ERBoundFinderU = function(null,  Y, D, Gamma)
{
  mean(Y - null*D - (Gamma-1)/(1+Gamma)*abs(Y-null*D))
}
ERBoundFinderL = function(null,  Y, D, Gamma)
{
  mean(Y - null*D - (Gamma-1)/(1+Gamma)*abs(Y-null*D))
}
